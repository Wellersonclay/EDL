...
<br><br> <p>
 <b> Expressividade </b>
   ...
   Neste exemplo, temos a utilização de um receptor de uma função literal:
   
   <pre>
   fun square(x: Int): Int =  x*xfun squared(a: Int, f: (Int) -> Int): Int = f(square(a))fun squared2(a: Int, f: Int.() -> Int): Int = f(square(a))
//use example
fun main(){
    println(square(5))                    // 25
...
O Kotlin nativo possui tipos de funções types , o que significa que variáveis podem, por exemplo, ser do tipo "função que aceita um Int e retorna um String ": (Int)->String . Esses tipos de função podem ser usados como parâmetros para outras funções, que são chamadas de "funções de ordem superior". Porém esta linguagem ainda permite mais, neste momento temos o conceito de receptores de funções literais "function literals with receiver" que propõe o conceito chamar métodos no receptor da função literal em seu corpo sem nenhum qualificador específico. Isso é muito semelhante às funções de extensão, onde também é possível acessar membros do objeto receptor dentro da extensão. Abaixo, mais um exemplo de função literal e seu receptor:

<pre>
<b> Função Literal </b>
println ( MinhaFuncaoDeAltaOrdem { "O número é $it" }) >> prints "O número é 5" 
</pre>
<pre> <b> Receptores de funções literais </b>
var greet: String.() -> Unit = { println("Olá $this") } 
 </pre>
Foi definida uma variável do tipo String.() -> Unit , que é basicamente um tipo de função () -> Unit com String como o receptor. Todos os métodos deste receptor podem, portanto, ser chamados no corpo do método. Neste exemplo, this é simplesmente usado para imprimir a String. Neste segundo exemplo, a função pode ser chamada da seguinte forma:
<pre>
greet("mundo") >> prints "Olá mundo" 
</pre>

<h2> Conclusão Kotlin Vs Java </h2>

Apesar de ser totalmente compativel com a linguagem Java, Kotlin foi capaz de corrigir alguns problemas nativos de Java o que garantiu para a linguagem um leque bem mais amplo e de novas possibilidades para a ultilização de recursos avançados, como o visto acima. Apesar do conceito de lambidas terem sido incorporados atualmente no Java, isso ainda se deu de maneira muito rasa, justamente pelos engessamentos estruturais presentes no Java.
Esta funcionalidade em especial, possibilidade a programação funcional utilizando Kotlin. Mas, não apenas por isto, é possível criar um código muito mais limpo, expressivo e simples, utilizando funções de alta ordem.


<h2> Bibliografia </h2>
. https://www.youtube.com/watch?v=9XYhEfA0Www
